...
setCategory(state,action){
    state.categoryL = action.payload;
}
    }
})
...
export const getCategory = () => async dispatch => {
    ...
    let response = await superagent.get(`https://app-auth-obieda.herokuapp.com/api/v1/categories`);
    dispatch(setCategory(response.body.results));
}

export const {category,setCategory} = categorySlice.actions;
